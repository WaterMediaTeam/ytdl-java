...
public class Utils {
    ...
    public static void createOutDir(final File outDir) throws IOException {
        if (!outDir.exists()) {
    ...
    // 1:32:54
    public static int parseLengthSeconds(final String text) {
        try {
            int length = 0;
            int beginIndex = 0;
            if (text.length() > 2) {
    ...
    public static long parseViewCount(final String text) {
        if (text == null || text.isEmpty()) {
            return 0;
        }
        final String value = text.replaceAll("[^0-9]", "");
        if (!value.isEmpty()) {
            try {
    ...
    public static String parseRuns(final JSONObject container) {
        if (container == null) {
            return null;
        }
        final JSONArray runs = container.getJSONArray("runs");
        if (runs == null) {
            return null;
        } else if (runs.size() == 1) {
    ...
    public static List<String> parseThumbnails(final JSONObject container) {
        if (container == null) {
            return null;
        }
        final JSONArray jsonThumbnails = container.getJSONArray("thumbnails");
        if (jsonThumbnails == null) {
            return null;
        } else {
            final List<String> thumbnails = new ArrayList<String>(jsonThumbnails.size());
            for (int i = 0; i < jsonThumbnails.size(); i++) {
                final JSONObject jsonThumbnail = jsonThumbnails.getJSONObject(i);
